...
\emph{primärer Schlüssel}\index{primärer Schlüssel: engl. "primary key", abgekürzt "pk" -- ein Attribut, das einen Datensatz eindeutig identifiziert}
...
ist dem Handbuch zum Server (\siehe{chap:Anhang-1}) zu entnehmen.
...
\emph{Custom-Fields}\index{Custom-Fields: Benutzerdefinierte Eigenschaften eines Objektes in der Datenbank, die für jedes Objekt unabhängig definierbar sind.}
...
Das für
\emph{Custom-Fields}\index{Custom-Fields: Benutzerdefinierte Eigenschaften eines Objektes in der Datenbank, die für jedes Objekt unabhängig definierbar sind.}
erforderliche Mixin (\siehe{designgrundlagen})
...
\emph{SAP ERP}
\index{SAP ERP: Enterprise-Resource-Planning Software der Firma SAP. Damit können Unternehmen mehrere Bereiche wie beispielsweise Inventardaten oder Kundenbeziehungen zentral verwalten}
System der HTL Rennweg (\siehe{import-aus-dem-sap-erp-system}) und und
intern von Lehrkräften und Inventarverantwortlichen geführte Listen
(\siehe{import-aus-sekunduxe4rer-und-tertiuxe4rer-quelle}).
...
Handbuch zum Server (\siehe{chap:Anhang-1}) zu entnehmen.
...
dem Handbuch zum Server (\siehe{chap:Anhang-1}) zu entnehmen.

\hypertarget{das-stocktaking-modul}{%
\section{Das Stocktaking-Modul}\label{das-stocktaking-modul}}

Das \emph{Stocktaking}\index{Stocktaking: Inventur}-Modul ermöglicht das
...
rückverfolgt werden können (\siehe{uxe4nderungsvorschluxe4ge}).
...
der \abb{fig:stocktaking_klassendiagramm} ist die Hierarchie in Form
...
\texttt{ChangeProposalBase} in \abb{fig:stocktaking_klassendiagramm}
...
Handhabung sind dem Handbuch zum Server (\siehe{chap:Anhang-1}) zu
entnehmen.
...
\emph{Custom-Fields}\index{Custom-Fields: Benutzerdefinierte Eigenschaften eines Objektes in der Datenbank, die für jedes Objekt unabhängig definierbar sind.}
und Anhängen (\siehe{speichern-von-bildern-und-anhuxe4ngen}). Weitere
Informationen zu den \texttt{Mixin}-Klassen kann der im Source-Code
enthaltenen Dokumentation entnommen werden.
...
In dem in \kap{die-client-schnittstelle} angegebenen Beispiel wird
...
\kap{barcode-eines-gegenstandes}).
...
implementiert (\siehe{api-und-drf}). Über diese Schnittstelle können
...
Um einen Gegenstand in einem Subraum (\siehe{subruxe4ume}) zu
...
\kap{das-stocktakingroomvalidation-modell} und
\kap{das-stocktakingitem-modell} behandelte \texttt{GenericForeignKey}
