...
\item
  ...
  oder
  \emph{CSRF}\index{CSRF: Cross-Site-Request-Forgery - eine Angriffsart, bei dem ein Opfer dazu gebracht wird, eine von einem Angreifer gefälschte Anfrage an einen Server zu schicken \cite{csrf}}
  \cite{csrf}), siehe Abschnitt \href{views}{``Views''}
\end{itemize}
...
\NormalTok{q }\OperatorTok{=}\NormalTok{ Entry.objects.}\BuiltInTok{filter}\NormalTok{(headline__startswith}\OperatorTok{=}\StringTok{"What"}\NormalTok{)}

\CommentTok{# Filtere aus dem erstellten Queryset alle Entry-Objekte, }
\CommentTok{# dessen Attribut "pub_date" kleiner oder gleich }
\CommentTok{# dem aktuellen Datum ist.}
\NormalTok{q }\OperatorTok{=}\NormalTok{ q.}\BuiltInTok{filter}\NormalTok{(pub_date__lte}\OperatorTok{=}\NormalTok{datetime.date.today())}

\CommentTok{# Schließe aus dem erstellten Queryset alle Entry-Objekte, }
\CommentTok{# dessen Attribut "body_text" den Text "food" beinhaltet, aus.}
\NormalTok{q }\OperatorTok{=}\NormalTok{ q.exclude(body_text__icontains}\OperatorTok{=}\StringTok{"food"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{unique_together }\OperatorTok{=}\NormalTok{ [[}\StringTok{"anlage"}\NormalTok{,  }\StringTok{"asset_subnumber"}\NormalTok{, }\StringTok{"company_code"}\NormalTok{]]}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{uxe4nderungsverlauf}{%
...
\emph{Hash-Werten}\index{Hash: Eine Funktion, die für einen Input immer einen (theoretisch) einzigartigen und gleichen Wert generiert.}
verglichen. Ist eine Datei mit dem
\emph{Hash-Wert}\index{Hash: Eine Funktion, die für einen Input immer einen (theoretisch) einzigartigen und gleichen Wert generiert.}
...
Bezüglich der Einzigartigkeit von \texttt{HTLRoom} Objekten gelten
...
\begin{Shaded}
\begin{Highlighting}[]
\AttributeTok{@receiver}\NormalTok{(post_save, sender}\OperatorTok{=}\NormalTok{HTLItemType)}
\KeywordTok{def}\NormalTok{ populate_htlitem_custom_field_values(sender, instance, }\OperatorTok{**}\NormalTok{kwargs):}
\NormalTok{    populate_inheritants_custom_field_values(instance)}
    
\AttributeTok{@receiver}\NormalTok{(post_save, sender}\OperatorTok{=}\NormalTok{HTLItem)}
\KeywordTok{def}\NormalTok{ populate_htlitemtype_custom_field_values(sender, instance, }\OperatorTok{**}\NormalTok{kwargs):}
\NormalTok{    populate_with_parents_custom_field_values(instance)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# Diese bekommt eine eigene "resource_class".}
...
\NormalTok{    resource_class }\OperatorTok{=}\NormalTok{ HTLItemSecodaryResource}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{ALIASES_HTLITEM }\OperatorTok{=}\NormalTok{ \{}
    \StringTok{"field_1"}\NormalTok{: ([}\StringTok{"Erstes Attribut"}\NormalTok{], \{}

\NormalTok{    \}),}
    \StringTok{"field_2"}\NormalTok{: ([}\StringTok{"Zweites Attribut"}\NormalTok{], \{}
...
\index{SAP ERP: Enterprise-Resource-Planning Software der Firma SAP. Damit können Unternehmen mehrere Bereiche wie beispielsweise Inventardaten oder Kundenbeziehungen zentral verwalten}
System importiert werden und beinhalten u.a. Raumänderungen, die durch
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{dev_ralph}\NormalTok{ graph_models stocktaking \textbackslash{}}
...
\NormalTok{-g -o stocktaking_klassendiagramm.png}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{unique_together }\OperatorTok{=}\NormalTok{ [[}\StringTok{"user"}\NormalTok{, }\StringTok{"stocktaking"}\NormalTok{]]}
\end{Highlighting}
\end{Shaded}
...
Handhabung sind dem Handbuch zum Server zu entnehmen.
...
\NormalTok{    item_model }\OperatorTok{=}\NormalTok{ BackOfficeAsset}
\NormalTok{    pk_url_kwarg }\OperatorTok{=} \VariableTok{None}
\NormalTok{    slug_url_kwarg }\OperatorTok{=} \StringTok{"barcode_final"}
\NormalTok{    slug_field }\OperatorTok{=} \StringTok{"barcode_final"}

\NormalTok{    display_fields }\OperatorTok{=}\NormalTok{ (}\StringTok{"hostname"}\NormalTok{, }\StringTok{"status"}\NormalTok{, }\StringTok{"location"}\NormalTok{, }\StringTok{"price"}\NormalTok{, }\StringTok{"provider"}\NormalTok{)}
\NormalTok{    extra_fields }\OperatorTok{=}\NormalTok{ (}
        \StringTok{"custom_fields"}\NormalTok{, }\StringTok{"office_infrastructure"}\NormalTok{, }\StringTok{"depreciation_rate"}\NormalTok{,}
        \StringTok{"budget_info"}\NormalTok{, }\StringTok{"property_of"}\NormalTok{, }\StringTok{"start_usage"}
\NormalTok{    )}
\NormalTok{    explicit_readonly_fields }\OperatorTok{=}\NormalTok{ (}\StringTok{"hostname"}\NormalTok{, }\StringTok{"price"}\NormalTok{)}

    \KeywordTok{def}\NormalTok{ get_queryset(}\VariableTok{self}\NormalTok{, request}\OperatorTok{=}\VariableTok{None}\NormalTok{, previous_room_validation}\OperatorTok{=}\VariableTok{None}\NormalTok{):}
        \CommentTok{# annotate custom barcode as either "barcode" or "sn"}
        \ControlFlowTok{return} \BuiltInTok{super}\NormalTok{(BackOfficeAssetStocktakingView, }\VariableTok{self}\NormalTok{).get_queryset(}
\NormalTok{            request}\OperatorTok{=}\NormalTok{request, previous_room_validation}\OperatorTok{=}\NormalTok{previous_room_validation}
\NormalTok{        ).annotate(}
\NormalTok{            barcode_final}\OperatorTok{=}\NormalTok{Case(}
\NormalTok{                When(Q(barcode__isnull}\OperatorTok{=}\VariableTok{True}\NormalTok{) }\OperatorTok{|}\NormalTok{ Q(barcode__exact}\OperatorTok{=}\StringTok{""}\NormalTok{),}
\NormalTok{                     then}\OperatorTok{=}\NormalTok{(F(}\StringTok{"sn"}\NormalTok{))),}
\NormalTok{                default}\OperatorTok{=}\NormalTok{F(}\StringTok{"barcode"}\NormalTok{), output_field}\OperatorTok{=}\NormalTok{CharField()))}

    \KeywordTok{def}\NormalTok{ get_room_for_item(}\VariableTok{self}\NormalTok{, item_object):}
        \ControlFlowTok{return} \BuiltInTok{str}\NormalTok{(item_object.warehouse)}

    \KeywordTok{def}\NormalTok{ item_display_name_getter(}\VariableTok{self}\NormalTok{, item_object):}
        \ControlFlowTok{return} \BuiltInTok{str}\NormalTok{(item_object)}

    \KeywordTok{def}\NormalTok{ item_barcode_getter(}\VariableTok{self}\NormalTok{, item_object):}
        \ControlFlowTok{return} \BuiltInTok{str}\NormalTok{(}
\NormalTok{            item_object.barcode_final}
\NormalTok{        ) }\KeywordTok{or} \StringTok{""} \ControlFlowTok{if} \BuiltInTok{hasattr}\NormalTok{(}
\NormalTok{            item_object, }\StringTok{"id"}
...
\NormalTok{    room_model }\OperatorTok{=}\NormalTok{ Warehouse}
\NormalTok{    item_detail_view }\OperatorTok{=}\NormalTok{ BackOfficeAssetStocktakingView()}

\NormalTok{    item_room_field_name }\OperatorTok{=} \StringTok{"warehouse"}
\end{Highlighting}
\end{Shaded}
...
Die Klassen \texttt{StocktakingGETWithCustomFieldsMixin},
\texttt{StocktakingPOSTWithCustomFieldsMixin},
\texttt{ClientAttachmentsGETMixin} und
\texttt{ClientAttachmentsPOSTMixin} ermöglichen das Miteinbeziehen von
\emph{Custom-Fields}\index{Custom-Fields: Benutzerdefinierte Eigenschaften eines Objektes in der Datenbank, die für jedes Objekt ünabhängig definierbar sind.}
und Änhängen (siehe Abschnitt
\protect\hyperlink{speichern-von-bildern-und-anhuxe4ngen}{``Speichern
von Bildern und Anhängen''}). Weitere Informationen zu den
\texttt{Mixin}-Klassen kann der im Source-Code enthaltenen Dokumentation
entnommen werden.
...
\cite{django-doku-querysets} hinzugefügt.
...
\NormalTok{        WarehouseStocktakingView.as_view(),}
\NormalTok{        name}\OperatorTok{=}\StringTok{"warehousesforstocktaking"}\NormalTok{),}
\NormalTok{    url(}
        \VerbatimStringTok{r"^backofficeassetsforstocktaking/(?:(?P<barcode_final>[\textbackslash{}w/]+)/)?$"}\NormalTok{, }
\NormalTok{        BackOfficeAssetStocktakingView.as_view(),}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{/api/stocktaking}\NormalTok{/}\ExtensionTok{?date_finished__isnull}\NormalTok{=True}\KeywordTok{&}\VariableTok{time_finish_isnull=}\NormalTok{True}\KeywordTok{&}\VariableTok{format=}\NormalTok{json}
\end{Highlighting}
\end{Shaded}
...
\OtherTok{[}
    \FunctionTok{\{}
        \DataTypeTok{"stocktake_id"}\FunctionTok{:} \DecValTok{1}\FunctionTok{,}
        ...
        \DataTypeTok{"neverending_stocktaking"}\FunctionTok{:} \KeywordTok{true}\FunctionTok{,}
    \FunctionTok{\}}\OtherTok{,}
    \FunctionTok{\{}
        \DataTypeTok{"stocktake_id"}\FunctionTok{:} \DecValTok{2}\FunctionTok{,}
        \DataTypeTok{"name"}\FunctionTok{:} \StringTok{"Inventur 2"}\FunctionTok{,}
        \DataTypeTok{"comment"}\FunctionTok{:} \StringTok{""}\FunctionTok{,}
        \DataTypeTok{"neverending_stocktaking"}\FunctionTok{:} \KeywordTok{false}\FunctionTok{,}
    \FunctionTok{\}}
\OtherTok{]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{STOCKTAKING_SERIALIZER_VIEWS }\OperatorTok{=}\NormalTok{ \{}
...
\FunctionTok{\{}
    \DataTypeTok{"displayFields"}\FunctionTok{:} \FunctionTok{\{}
        \DataTypeTok{"anlagenbeschreibung_prio"}\FunctionTok{:} \FunctionTok{\{}
        ...
        \FunctionTok{\},}
        \DataTypeTok{"item_type"}\FunctionTok{:} \FunctionTok{\{}
    ...
    \DataTypeTok{"extraFields"}\FunctionTok{:} \FunctionTok{\{}
        \DataTypeTok{"anlagenbeschreibung"}\FunctionTok{:} \FunctionTok{\{}
            ...
            \DataTypeTok{"type"}\FunctionTok{:} \StringTok{"string"}
        \FunctionTok{\},}
        \DataTypeTok{"custom_fields"}\FunctionTok{:} \FunctionTok{\{}
        ...
        \DataTypeTok{"sponsor"}\FunctionTok{:} \FunctionTok{\{}
            ...
            \DataTypeTok{"type"}\FunctionTok{:} \StringTok{"string"}
        \FunctionTok{\},}
        \DataTypeTok{"is_in_sap"}\FunctionTok{:} \FunctionTok{\{}
...
\FunctionTok{\{}
    ...
    \DataTypeTok{"items"}\FunctionTok{:} \OtherTok{[}\FunctionTok{\{}
            \DataTypeTok{"times_found_last"}\FunctionTok{:} \DecValTok{1}\FunctionTok{,}
            \DataTypeTok{"itemID"}\FunctionTok{:} \DecValTok{2}\FunctionTok{,}
            \DataTypeTok{"displayName"}\FunctionTok{:} \StringTok{"Tischlampe"}\FunctionTok{,}
            \DataTypeTok{"displayDescription"}\FunctionTok{:} \StringTok{""}\FunctionTok{,}
            \DataTypeTok{"barcode"}\FunctionTok{:} \StringTok{"46010000"}\FunctionTok{,}
...
\FunctionTok{\{}
    ...
    \DataTypeTok{"validations"}\FunctionTok{:} \OtherTok{[}
        \FunctionTok{\{}
            ...
            \DataTypeTok{"mark_for_later_validation"}\FunctionTok{:} \KeywordTok{true}
        \FunctionTok{\}}
    \OtherTok{]}
\FunctionTok{\}}
\end{Highlighting}
...
\FunctionTok{\{}
    ...
    \DataTypeTok{"subroomValidations"}\FunctionTok{:} \OtherTok{[}\FunctionTok{\{}
            \DataTypeTok{"roomID"}\FunctionTok{:} \DecValTok{100}\FunctionTok{,}
            \DataTypeTok{"validations"}\FunctionTok{:} \OtherTok{[}\FunctionTok{\{}
                    \DataTypeTok{"itemID"}\FunctionTok{:} \DecValTok{1}
                \FunctionTok{\}}
            \OtherTok{]}\FunctionTok{,}
...
\emph{Pull-Request}\index{Pull-Request: Das Miteinbeziehen von individuell entwickeltem Quellcode in die offizielle Quell-Version der Software}
in das offizielle Ralph-System einzubinden. Durch der in den Abschnitten
\protect\hyperlink{das-stocktakingroomvalidation-modell}{``Das
StocktakingRoomValidation Modell''} und
\protect\hyperlink{das-stocktakingitem-modell}{``Das StocktakingItem
Modell''} behandelten \texttt{GenericForeignKey} Funktionalität
\cite{django-doku-contenttypes} ist es möglich, das Stocktaking-Modul
für jegliche Modellklassen zu verwenden. Es gibt keine Beschränkung auf
die durch das Diplomarbeitsteam implementierten Klassen.
